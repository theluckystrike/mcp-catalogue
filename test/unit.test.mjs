...
test.skip("the PDF is written and names the currency and tier", async (t) => {
...
test.skip("the report counts rows in force, rows a later row replaces, and rows missing the default currency", async (t) => {
...
test.skip("the register records what a resolution priced, and the report shows it", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  await seed(c);

...
test.skip("forty SKUs set from two processes are all stored, with forty distinct codes", async (t) => {
...
test.skip("thirty price rows added to one SKU from two processes are all kept", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
...
test.skip("resolutions raced from two processes get distinct ids and one register row per ref", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
  await seed(a);
  const calls = [];
